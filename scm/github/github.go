...
type AccessTokenRequestQuery struct {
	ClientID     string `url:"client_id"`
	ClientSecret string `url:"client_secret"`
	AuthCode     string `url:"code"`
...
func RequestToken(clientID, clientSecret, authCode, state string) (token string, err error) {
	...
	req, err := sling.New().
		...
		Request()
	if err != nil {
		log.WithError(err).Debug("unable to create github request for access token")
		return
	}
	client := &http.Client{}
	res, err := client.Do(req)
	if err != nil {
		log.WithError(err).Debug("failed running github request")
		return
	}
	defer res.Body.Close()
	body, err := ioutil.ReadAll(res.Body)
	if err != nil {
		log.WithError(err).Debug("unable to read response")
		return
	}
	var jsonBody GithubAuthResponse
	if err = json.Unmarshal(body, &jsonBody); err != nil {
		log.WithError(err).Debug("unable to unmarshal response")
		return
	}
...
func (c *Client) GetAuthenticatedUser() (*github.User, error) {
	user, res, err := c.githubClient.Users.Get("")
	if err != nil {
		log.WithError(err).Debug("unable to get authenticated user")
		return nil, err
	}
	defer res.Body.Close()
	return user, nil
...
func (c *Client) GetAuthenticatedUserEmails() ([]github.UserEmail, error) {
	emails, res, err := c.githubClient.Users.ListEmails(nil)
	if err != nil {
		log.WithError(err).Debug("unable to get authenticated user's email list")
		return nil, err
	}
	defer res.Body.Close()
	return emails, nil
